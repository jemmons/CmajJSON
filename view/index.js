...
*/
class JSONTest_View extends HTMLElement
{
    ...
    {
        this.data = await fetch('./view/MagicNumbers.json').then(res => res.json());        
    ...
    getHTML()
    {
        ...
        <div id="controls">
          <p>Magic Number is: ${this.data[1024]}</p>
          <input type="range" class="param" id="frequency" min="5" max="1000">Frequency</input>
        </div>`;
    }
}

window.customElements.define ("jsontest-view", JSONTest_View);
...
{
    return new JSONTest_View (patchConnection);
}
